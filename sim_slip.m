function out=sim_slip()
% Simulation
% The SLIP Scheduling Algorithm
...
% average delay
Avg_delay_slip = zeros(1,length(rho));
...
for r = 1:length(rho);
    ...
    while t<=T
        ...
        for y = 1:N % for every input y, send requests to output
            ...
            % slip Request: '1' means a request, '0' means no request
            Req_slip(y,:) = voq_l(y,:)>0;
            
        end
        
        % generate grants
        for u = 1:N  % for every output u, randomly select an input for grant
            if sum(Req_slip(:,u))>0  % has at least one Request
                
                % randomly select an input to send output u
                rnd_k = rand(1,N).*Req_slip(:,u)'; % generate random numbers                
        ...
        for y = 1:N   % for every input y, select a grant
            if (sum(Gra_slip==y)>0)  && (any(Gra_slip))
    ...
    Avg_delay_slip(r) = total_delay/total_pkt;
    
end

out=[rho;Avg_delay_slip];
% save slip.mat out;
% 
% figure(1)
% semilogy(rho,Avg_delay_slip);
